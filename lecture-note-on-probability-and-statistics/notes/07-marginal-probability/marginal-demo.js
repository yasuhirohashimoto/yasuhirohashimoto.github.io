...
// - Sampling / binning / contour grid: generated in cancellable chunks
...
(async () => {
	...
	const liveStatus = document.getElementById("marginal-live");
	...
	// A dark shared palette colour lets overlapping semi-transparent points
	// approach an opaque tone. With clearColor (0,0,0,0) and
	...
	async function generateData(sx, sy, covRaw, nextN, nBins, isCancelled) {
		const maxCov = sx * sy;
	...
	function initSvg() {
		...
		// Small permanent legend in the empty top-right corner between the
		// two histogram bands: blue = marginal, orange = conditional.
	...
	function renderContours() {
		try {
			...
			// Two-layer casing: a light halo under each line keeps the contours
			// readable where the point cloud saturates toward black.
			ds.forEach(d => {
	...
	function renderHistograms() {
		...
		// Hover hit bands: one transparent rect over each histogram so that
		// pointer tracking has no dead zones (bar gaps / area above short bars)
		// and the highlight does not flicker while the pointer moves across bins.
	...
	function onBinEnter(axis, bin) {
		...
		// Highlight the hovered bar and fade the remaining base bars so the
		// orange conditional histogram reads as the foreground (restored in
		// clearInteractionState).
	...
	// are coalesced, and an in-progress chunked generation is cancelled as soon
	// as a newer input arrives.
	...
	// (log10 N) so the low range stays adjustable, while the number input
	// and the generated data use real counts.
