(() => {
	...
	function createChart({ id, yMode, showYLabel = true }) {
		...
		g.append("g")
			...
			.call(styleAxis)
			.attr("font-size", 16);
		g.append("g").call(yAxis).call(styleAxis).attr("font-size", 16);
		...
		texFO(
			...
			16,
			"\\(n\\)（対数軸）",
			{ anchor: "topcenter", color: PROB_COLORS.text, size: "16px" }
		);
		...
		if (showYLabel) {
			...
			texFO(
				...
				24,
				"\\(\\bar{X}_n\\)",
				{ anchor: "center", color: PROB_COLORS.text, size: "16px" }
			);
		}
		...
		g.append("line")
			...
			.attr("stroke", PROB_COLORS.D).attr("stroke-width", 1.6)
			.attr("stroke-dasharray", "4 3");

		texFO(
			svg,
			margin.left + innerW - 96,
			margin.top + yPosition(target) - 20,
			96,
			18,
			"\\(E[X] = 0.5\\)",
			{ anchor: "topleft", color: PROB_COLORS.DText, size: "16px" }
		);
