(function () {
	...
	function drawCenterLine(svg, value, y0, y1, color, label) {
		const x = xScale(value);
		svg.appendChild(el("line", {
			x1: x,
			y1: y0,
	...
	function drawDeviationRows(svg, sample, center, yBase, color, label, value) {
		...
		drawCenterLine(svg, center, yBase - 10, yBase + rowSpan + 4, color, label.includes("μ") ? "μ" : "X̄");
	...
	function render() {
		...
		drawCenterLine(svg, mu, 24, 60, colors.mu, "μ");
		svg.appendChild(el("text", {
			x: plotRight,
			y: 26,
			"text-anchor": "end",
		...
		drawAxis(svg, 126, hasSample ? "標本（最新の1回）" : "標本");
		if (hasSample) {
			...
			drawCenterLine(svg, xbar, 90, 128, colors.xbar, "X̄");
		}
		svg.appendChild(el("text", {
			x: plotRight,
			y: 94,
		...
		if (hasSample) {
			drawDeviationRows(svg, state.sample, mu, 174, colors.lineMu, "μ 中心の平均二乗偏差", qMu);

			drawDeviationRows(svg, state.sample, xbar, 236, colors.lineXbar, "X̄ 中心の平均二乗偏差 S²", s2);
		}
