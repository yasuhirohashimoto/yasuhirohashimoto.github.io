(async () => {
			...
			arrowPath.setAttribute("fill", PROB_COLORS.text);
			...
			GRID_VALUES.forEach(v => {
				...
				vline.setAttribute("stroke", PROB_COLORS.grid); vline.setAttribute("stroke-width", 0.8);
				...
				hline.setAttribute("stroke", PROB_COLORS.grid); hline.setAttribute("stroke-width", 0.8);
			...
			xAxis.setAttribute("stroke", PROB_COLORS.text); xAxis.setAttribute("stroke-width", 1.2);
			...
			yAxis.setAttribute("stroke", PROB_COLORS.text); yAxis.setAttribute("stroke-width", 1.2);
			...
			ticks.forEach(v => {
				...
				xtLine.setAttribute("stroke", PROB_COLORS.text); xtLine.setAttribute("stroke-width", 1);
				...
				ytLine.setAttribute("stroke", PROB_COLORS.text); ytLine.setAttribute("stroke-width", 1);
			...
			for (let j = 0; j < DINO.length; j += 2) {
				...
				dot.setAttribute("fill", PROB_COLORS.DC);
			...
			burst.setAttribute("fill", PROB_COLORS.DC);
			burst.setAttribute("stroke", PROB_COLORS.node);
			...
			burstText.setAttribute("font-size", "13");
			burstText.setAttribute("font-weight", "bold");
			burstText.setAttribute("fill", PROB_COLORS.node);
			burstText.textContent = "相関なしウス！";
			svg.appendChild(burstText);

			// MathJax labels: X, Y, O, and tick numbers (50, 100)
			// texFO（assets/diagram-helpers.js）は素の SVG 要素も受けられる（このノートは D3 を読み込まない）
			texFO(svg, X_TIP + 12, ORIGIN_Y,        18, 18, "\\(X\\)", { anchor: "center", color: PROB_COLORS.text, size: "14px" });
			texFO(svg, ORIGIN_X,   Y_TIP - 12,      18, 18, "\\(Y\\)", { anchor: "center", color: PROB_COLORS.text, size: "14px" });
			texFO(svg, ORIGIN_X - 10, ORIGIN_Y + 10, 14, 14, "\\(O\\)", { anchor: "center", color: PROB_COLORS.text, size: "12px" });
			ticks.forEach(v => {
				texFO(svg, xToPx(v), ORIGIN_Y + 10, 30, 16, String(v), { anchor: "center", color: PROB_COLORS.sub, size: "13px" });
				texFO(svg, ORIGIN_X - 14, yToPx(v), 30, 16, String(v), { anchor: "center", color: PROB_COLORS.sub, size: "13px" });
			});
